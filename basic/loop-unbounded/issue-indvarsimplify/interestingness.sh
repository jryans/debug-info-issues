...
if
  ...
  grep -F 'example(int n)' ${SRC_FILE} && \
  grep -F 'int y = 0;' ${SRC_FILE} && \
  grep -F 'int i = 0;' ${SRC_FILE} && \
  grep -F 'i < n;' ${SRC_FILE} && \
  grep -F 'i++' ${SRC_FILE} && \
  grep -E 'return.*\by' ${SRC_FILE} && \
  ...
  ! grep -F 'set but not used' reduce.tmp/cc-O0 && \
  ...
  ! grep -F 'was not declared' reduce.tmp/cc-O0 && \
  ...
  grep -F '@dbg.value(i32 undef, "y" l3)' reduce.tmp/O1-cg-di.ll && \
  grep -F '@dbg.value(i32 undef, "i" l4)' reduce.tmp/O1-cg-di.ll
then
  exit 0
else
  exit 1
