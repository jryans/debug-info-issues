...
int main() {
#ifdef CONCRETE
  uint32_t hashes[4] = {0};
  struct bloom_key key = {(uint32_t *)&hashes};
  unsigned char data[4] = {0};
  struct bloom_filter filter = {(unsigned char *)&data, 4};
  struct bloom_filter_settings settings = {.num_hashes = 4};
#else
  ...
  klee_make_symbolic(&settings, sizeof(struct bloom_filter_settings),
                     "settings");
#endif
