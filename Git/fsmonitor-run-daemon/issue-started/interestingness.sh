...
if
  ...
  grep -F 'health_started = 0' ${SRC_FILE} && \
  grep -F 'health_started = 1' ${SRC_FILE} && \
  grep -F 'goto cleanup' ${SRC_FILE} && \
  grep -F 'if (health_started)' ${SRC_FILE} && \
  ...
  ! grep -F "used uninitialized" reduce.tmp/cc-O0 && \
  ...
  ! grep -F 'incompatible integer' reduce.tmp/cc-O0 && \
  ...
  grep -F '@dbg.value(i32 undef, "health_started"' reduce.tmp/O1-cg-di.ll
then
  exit 0
else
  exit 1
