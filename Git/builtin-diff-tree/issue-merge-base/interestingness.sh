...
if
  ...
  grep -F 'int merge_base' ${SRC_FILE} && \
  grep -F 'merge_base =' ${SRC_FILE} && \
  grep -F 'if (merge_base)' ${SRC_FILE} && \
  grep -F 'diff_get_merge_base(' ${SRC_FILE} && \
  ...
  grep -F '@dbg.value(i32 undef, "merge_base"' reduce.tmp/O1-cg-di.ll
then
  exit 0
else
  exit 1
