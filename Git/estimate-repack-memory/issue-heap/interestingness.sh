...
if
  ...
  grep -F 'heap =' ${SRC_FILE} && \
  grep -F 'heap +=' ${SRC_FILE} && \
  grep -F 'return heap' ${SRC_FILE} && \
  ...
  grep -F '@dbg.value(!DIArgList(i64 %call, i64 %div), "heap"' reduce.tmp/O1-cg-di.ll && \
  grep -F '@dbg.value(i64 undef, "heap"' reduce.tmp/O1-cg-di.ll
then
  exit 0
else
  exit 1
